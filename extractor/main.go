...
var (
	...
	jobCounter = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			...
			Help: "Total number of extract jobs processed, by status.",
		},
	...
	jobDuration = prometheus.NewHistogram(
		prometheus.HistogramOpts{
			...
			Help:    "Duration of extract jobs in seconds.",
			Buckets: prometheus.DefBuckets,
		},
	)

	rowsProcessed = prometheus.NewCounter(
		prometheus.CounterOpts{
			Name: "extractor_rows_processed_total",
			Help: "Total number of crash rows processed by the extractor.",
		},
	)
	...
	stageDuration = prometheus.NewHistogramVec(
		prometheus.HistogramOpts{
			...
			Help:    "Duration of major extractor stages (api_fetch, minio_write, enrich_fetch).",
			Buckets: prometheus.DefBuckets,
		},
		[]string{"stage"},
	)
	...
	errorCounter = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			...
			Help: "Total number of errors in extractor, by stage.",
		},
		[]string{"stage"},
	)
	...
	successCounter = prometheus.NewCounter(
		prometheus.CounterOpts{
			Name: "extractor_success_total",
			Help: "Total number of successful extractor jobs.",
		},
	)
	...
	uptimeGauge = prometheus.NewGaugeFunc(
		prometheus.GaugeOpts{
			...
			Help: "Uptime of the extractor service in seconds.",
		},
		func() float64 {
	...
	// New: per-job success/fail counters (for dashboards)
	extractorJobsSuccess = prometheus.NewCounter(
		prometheus.CounterOpts{
			Name: "extractor_jobs_success_total",
			Help: "Number of extractor jobs that completed successfully.",
		},
	)

	extractorJobsFailed = prometheus.NewCounter(
		prometheus.CounterOpts{
			Name: "extractor_jobs_failed_total",
			Help: "Number of extractor jobs that failed.",
		},
	)

	// New: last successful run timestamp (Unix seconds)
	lastRun = prometheus.NewGauge(
		prometheus.GaugeOpts{
			Name: "extractor_last_success_timestamp",
			Help: "Unix timestamp of last successful extractor job.",
		},
	)

	// 🔥 New: count of MinIO objects (Bronze) written by extractor
	extractor_files_written = prometheus.NewCounter(
		prometheus.CounterOpts{
			Name: "extractor_files_written_total",
			Help: "Number of MinIO objects written by extractor (Bronze / raw-data).",
		},
	)
)

func init() {
	// Register metrics once at process start
	prometheus.MustRegister(
		...
		successCounter,
		uptimeGauge,
		extractorJobsSuccess,
		extractorJobsFailed,
		lastRun,
		extractor_files_written, // 👈 register new counter
	)
}
...
func httpGetJSON(env Env, fullURL string) ([]byte, error) {
	start := time.Now()
	defer func() {
		stageDuration.WithLabelValues("api_fetch").Observe(time.Since(start).Seconds())
	}()
	...
	for i := 0; i < 3; i++ {
		...
		if err != nil {
			lastErr = err
			errorCounter.WithLabelValues("api_fetch").Inc()
...
func putJSONGZ(cli *minio.Client, env Env, key string, raw []byte, meta map[string]string) error {
	start := time.Now()
	defer func() {
		stageDuration.WithLabelValues("minio_write").Observe(time.Since(start).Seconds())
	}()
	...
	_, err := gz.Write(raw)
	if err != nil {
		errorCounter.WithLabelValues("minio_write").Inc()
		return err
	}
	...
	// ✅ Count every successfully written Bronze object
	extractor_files_written.Inc()
...
func processJob(env Env, mcli *minio.Client, job Job) (string, bool, error) {
	...
	// count rows processed for metrics
	var totalRows int64
	defer func() {
		if totalRows > 0 {
			rowsProcessed.Add(float64(totalRows))
		}
	}()
	...
	for {
		...
		if err != nil {
			errorCounter.WithLabelValues("job").Inc()
		...
		if err := writePageMarker(mcli, env, mk, pageMax, corr, job.Primary.Alias); err != nil {
			...
			// marker failure is not considered job failure for now
		}
	...
	for w := 0; w < maxWorkers; w++ {
		...
		go func() {
			...
			for t := range tasks {
				...
				stageDuration.WithLabelValues("enrich_fetch").Observe(time.Since(start).Seconds())
			}
		}()
	}
	// Feed tasks
...
func fetchOneBatchAllPages(env Env, mcli *minio.Client, job Job, corr string, ds *DatasetSpec,
	...
	for off := 0; ; off += limit {
		...
		if err != nil {
			...
			errorCounter.WithLabelValues("enrich_fetch").Inc()
			return
		}
		var arr []map[string]any
		if err := json.Unmarshal(raw, &arr); err != nil {
...
func writeManifest(cli *minio.Client, env Env, m Manifest) {
	...
	if err != nil {
		...
		errorCounter.WithLabelValues("manifest_write").Inc()
	}
}
...
func main() {
	...
	go func() {
		...
		log.Printf("Prometheus metrics server listening on :8000/metrics")
	...
	for d := range msgs {
		...
		if err := json.Unmarshal(d.Body, &job); err != nil {
			...
			errorCounter.WithLabelValues("decode_job").Inc()
			jobCounter.WithLabelValues("error").Inc()
			extractorJobsFailed.Inc()
			d.Ack(false)
			continue
		}
		...
		if job.Primary.ID == "" {
			...
			errorCounter.WithLabelValues("job_validation").Inc()
			jobCounter.WithLabelValues("error").Inc()
			extractorJobsFailed.Inc()
			d.Ack(false)
			continue
		...
		if err != nil {
			...
			jobCounter.WithLabelValues("success").Inc()
			successCounter.Inc()
			extractorJobsSuccess.Inc()
		...
		// Ack the message once we’re done (success or failure)
		d.Ack(false)

		// Publish transform job only on success with data
		if err == nil && wroteAny && corr != "" {
			tj := TransformJob{
				Type:   "transform",
				CorrID: corr,
			}
			if err := publishTransformJob(env.RabbitURL, tj); err != nil {
				...
				errorCounter.WithLabelValues("publish_transform").Inc()
			} else {
